...
internal sealed class GetItemByIdQueryHandler(IItemRepository items) : IRequestHandler<GetItemByIdQuery, Result<GetItemByIdResult>>
{
    ...
    private static GetItemByIdResult Map(Item item) => new(
        item.ItemId,
        item.OwnerId,
        item.ObjectName,
        item.Maker,
